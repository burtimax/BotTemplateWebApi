...
public class BotDispatcherController : BaseBotController
{
    ...
    public override async Task<IActionResult> HandleBotRequest(Update update)
    {
        ...
        try
        {
            ...
            return await ProcessRequestByHandler(handlerType, update) ;
        }
        catch (Exception e)
        {
            return Ok();
    ...
    private Task<IActionResult> ProcessRequestByHandler (Type handlerType, Update update)
    {
        object? handlerInstance = _assembly.CreateInstance(handlerType.FullName, true, BindingFlags.Default, null, new object[]{ HttpContext.RequestServices}, null, null);
