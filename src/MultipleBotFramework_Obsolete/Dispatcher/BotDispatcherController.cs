...
using MultipleBotFramework_Obsolete.Dispatcher.HandlerResolvers;
using MultipleBotFramework_Obsolete.Dto;
using MultipleBotFramework_Obsolete.Exceptions;
using MultipleBotFramework_Obsolete.Extensions;
...
using MultipleBotFramework_Obsolete.Utils.ExceptionHandler;
using Newtonsoft.Json;
using Telegram.Bot;
using Telegram.Bot.Types;
using Telegram.Bot.Types.Enums;

namespace MultipleBotFramework_Obsolete.Dispatcher;
...
public class BotDispatcherController : BaseBotController
{
    ...
    public override async Task<IActionResult> HandleBotRequest(long botId, Update update)
    {
        ...
        try
        {
            ...
            // Тут обрабатываем уже состояние бота.
    ...
    private Task ProcessRequestByHandler<T>(Type handlerType, long botId, bool isOwner, ITelegramBotClient botClient, Update update, BotChatEntity? chat, BotUserEntity? user, BotUpdateEntity? savedUpdate, IEnumerable<ClaimValue>? userClaims) where T : IBaseBotHandler
    {
        ...
        T handlerInstance = (T) ActivatorUtilities.CreateInstance(HttpContext.RequestServices, handlerType, new object[] {HttpContext.RequestServices });
